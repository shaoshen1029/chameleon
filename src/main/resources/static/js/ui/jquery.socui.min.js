 ...
 */
(function($){
$socui={indexOfArray:function(a,o,id){
...
},plugins:["draggable","droppable","resizable","pagination","tooltip","prompt","badge","condition","radiobox","checkbox","tabbutton","steps","animatenumber","stackprogressbar","rangebox","nodeline","carousel","transfer","linkbutton","menu","menubutton","splitbutton","switchbutton","progressbar","tree","textbox","passwordbox","maskedbox","filebox","combo","combobox","combotree","combogrid","combotreegrid","tagbox","numberbox","validatebox","searchbox","spinner","numberspinner","timespinner","datetimespinner","calendar","datebox","datetimebox","slider","layout","panel","datagrid","propertygrid","treegrid","datalist","tabs","accordion","window","dialog","form","fastmenu","flowview","risklegend","timeline"],parse:function(_c){
// 渲染国际化资源内容(@see common.i18n.js)
// renderLocaleMessage();
var aa=[];
...
if(r1||r2){
$(this).addClass("socui-fluid");
}else{
$(this).removeClass("socui-fluid");
}
});
}
...
if($(this).hasClass("socui-fluid")||_a9){
_8f(this);
}
return false;
});
...
_114(_10e,_113,_110);
//cascadeModel自定义属性:childCascade-关联子节点;halfCascade-半关联选中;
if (opts.cascadeModel == "halfCascade") {
_115(_10e,_113);
}
}else{
...
_116(_117,_118,flag);
if(opts.deepCheck){
$socui.forEach(_118.children||[],true,function(n){
_116(_117,n,flag);
});
}else{
var _11a=[];
...
var c1=0;
var len=0;
$socui.forEach(row.children||[],false,function(r){
if(r.checkState){
len++;
...
$socui.forEach(data,true,function(node){
_175.push(_172(node));
});
return _175;
};
...
var _17b=[];
$socui.forEach($.data(_179,"tree").data,true,function(n){
if(n.checkState&&$socui.indexOfArray(_17a,n.checkState)!=-1){
_17b.push(_172(n));
}
});
return _17b;
...
$socui.forEach(data.children,true,function(node){
_172(node);
});
}
return data;
...
var ids={};
$socui.forEach(_19c.data,true,function(node){
...
var t=$(_1bc);
return $.extend({},$.parser.parseOptions(_1bc,["url","method","cascadeModel",{checkbox:"boolean",cascadeCheck:"boolean",onlyLeafCheck:"boolean"},{animate:"boolean",lines:"boolean",dnd:"boolean"}]));
};
...
if(item.checked){
$socui.addArrayItem(_1c3.tmpIds,item.domId);
}
}
...
$.fn.tree.defaults={url:null,method:"post",animate:false,checkbox:false,cascadeCheck:true,cascadeModel:"childCascade",onlyLeafCheck:false,lines:false,dnd:false,data:null,queryParams:{},formatter:function(node){
return node.text;
...
if($(this).hasClass("socui-fluid")||_1e1){
_1e2(_1e0);
}
return false;
});
...
if($(this).hasClass("socui-fluid")||_22b){
_212(_229,{});
}
return false;
});
...
$(_239).panel("clear");
$(_239).html($("<div class=\"panel-loading\"><i class=\"soc-icon-font soc-icon-refresh soc-spin\"></i><i>" + opts.loadingMessage + "</i></div>"));
}
opts.loader.call(_239,_23c,function(data){
var _23d=opts.extractor.call(_239,data);
$(_239).panel("clear");
//先隐藏面板,放置出现为渲染组件的页面
$(_239).html($("<div class=\"panel-loading\" style=\"position: absolute;top: 46px;left: 0;\"><i class=\"soc-icon-font soc-icon-refresh soc-spin\"></i><i>loading...</i></div>"));
$(_239).append('<div class="panel-content" style="visibility:hidden;">' + _23d + '</div>');
$.parser.parse($(_239));
//渲染后显示面板
$(_239).find('div[class="panel-loading"]').remove();
$(_239).find('div[class="panel-content"]').css('visibility', 'visible');
opts.onLoad.apply(_239,arguments);
_23b.isLoaded=true;
//集成jquery.slimScroll滚动条
$('.bh-dialog-layout').slimScroll();
},function(){
...
if(_267.length){
_267.layout("resize");
$("body").children(".socui-fluid:visible").each(function(){
...
var s=$(_272).find("div.panel:visible,div.accordion:visible,div.tabs-container:visible,div.layout:visible,.socui-fluid:visible").filter(function(_274,el){
...
opts.top=Math.ceil(($(window).outerHeight()-_28d)/2+$(document).scrollTop());
}
if(_28c){
_281(_28b);
}
...
// 调整模态窗口遮罩层,保证页面只有一个遮罩层。解决原组件弹出多层遮罩层的BUG（通过设置遮罩层z-index属性实现）
var tmpShadow = $("div[class=\"window-shadow\"]");
if(tmpShadow.length > 0){
	if(_291.mask){
		var tmpIndex = $.fn.window.defaults.zIndex - 6;
		$.fn.window.defaults = $.extend($.fn.window.defaults, {zIndex:$.fn.window.defaults.zIndex - 3});
		_291.mask.css($.extend({display:"block",zIndex:tmpIndex},$.fn.window.getMaskSize(_290)));
	}
	// 获取当前遮罩层z-index值,用于判断小于此z-index的窗口,移除其高斯模糊
	var window_index = $('div[class="window-mask"]').css('z-index');
	$('div.panel.window.panel-htop').each(function() {
		if ($(this).css('z-index') > window_index) {
			$(this).removeClass('window-blur');
		}
	});
}else{
	if(_291.mask){
		// 移除背景高斯模糊
		$('div.bh-bg').removeClass('window-blur');
		_291.mask.remove();
	}
}
},onClose:function(){
...
if (!$('div.bh-bg').hasClass('window-blur')) {
	$('div.bh-bg').addClass('window-blur');
}
// 获取当前遮罩层z-index值,用于判断小于此z-index的窗口,增加其高斯模糊
var window_index = $('div[class="window-mask"]').css('z-index');
$('div.panel.window.panel-htop').each(function() {
	if ($(this).css('z-index') < window_index) {
		$(this).addClass('window-blur');
	}
});
...
var pmaskIndex=$.fn.window.defaults.zIndex+1;
_29e.pmask.css({display:"none",zIndex:pmaskIndex,left:e.data.left,top:e.data.top,width:_29e.window._outerWidth(),height:_29e.window._outerHeight()});
if(_29e.proxy){
_29e.proxy.remove();
}
_29e.proxy=$("<div class=\"window-proxy\"></div>").insertAfter(_29e.window);
var proxyIndex=$.fn.window.defaults.zIndex+2;
_29e.proxy.css({display:"none",zIndex:proxyIndex,left:e.data.left,top:e.data.top});
...
bb.insertAfter(_2b4);
tb.add(bb)._outerWidth(t._outerWidth()).find(".socui-fluid:visible").each(function(){
$(this).triggerHandler("_resize");
});
...
if($(this).hasClass("socui-fluid")||_2f6){
_2d4(_2f4);
}
return false;
});
...
_333.add(_334).css('line-height', (_335-1)+"px");
...
p_t.find(".socui-fluid:visible").triggerHandler("_resize");
};
};
function _348(_349){
...
if($(this).hasClass("socui-fluid")||_351){
_33c(_34f);
_348(_34f);
}
return false;
...
if($(this).hasClass("socui-fluid")||_3c8){
_3ba(_3c4);
}
return false;
});
...
if($(this).hasClass("socui-fluid")||_422){
...
menu.find(".socui-fluid").triggerHandler("_resize",[true]);
...
(function($){
function init(_493){
var _494=$("<span class=\"switchbutton\">"+"<span class=\"switchbutton-inner\">"+"<span class=\"switchbutton-on\"></span>"+"<span class=\"switchbutton-handle\"></span>"+"<span class=\"switchbutton-off\"></span>"+"<input class=\"switchbutton-value\" type=\"checkbox\">"+"<input class=\"switchbutton-default\" type=\"hidden\">"+"</span>"+"</span>").insertAfter(_493);
var t=$(_493);
...
_494.find(".switchbutton-default").attr("name",name);
}
_494.bind("_resize",function(e,_495){
if($(this).hasClass("socui-fluid")||_495){
_496(_493);
}
return false;
});
...
_4a2.find(".switchbutton-default").attr("value", opts.defaultValue);
...
$(_4a6).add(_4ad).attr("value",opts.checked);
if(ck!=opts.checked){
opts.onChange.call(_4a6,opts.checked);
}
opts.onClick.call(_4a6,opts.checked);
};
...
$.fn.switchbutton.defaults={handleWidth:"auto",width:60,height:26,checked:false,disabled:false,readonly:false,reversed:false,onText:"ON",offText:"OFF",onValue:'true',offValue:'false',handleText:"",defaultValue:"true",value:"on",onChange:function(_4c0){
},onClick:function(_4c0){
}};
})(jQuery);
(function($){
function init(_4c1){
...
opts.err(_4c9, _4ca.message, "hide");
if($(_4c9).attr("readonly")){
return;
}
//增加焦点验证开关,默认false,焦点不验证,失去焦点验证
if (opts.isFocus === true) {
...
var t=$(_4f1);
return $.extend({},$.parser.parseOptions(_4f1,["validType","missingMessage","invalidMessage","tipPosition",{delay:"number",interval:"number",deltaX:"number"},{editable:"boolean",validateOnCreate:"boolean",validateOnBlur:"boolean",isFocus:"boolean"}]),{required:(t.attr("required")?true:undefined),disabled:(t.attr("disabled")?true:undefined),readonly:(t.attr("readonly")?true:undefined),novalidate:(t.attr("novalidate")!=undefined?true:undefined)});
};
$.fn.validatebox.defaults={required:false,validType:null,validParams:null,delay:200,interval:200,missingMessage:"This field is required.",invalidMessage:null,tipPosition:"right",deltaX:0,deltaY:0,novalidate:false,editable:true,disabled:false,readonly:false,validateOnCreate:true,validateOnBlur:false,isFocus:false,events:{focus:_4c8,blur:_4cc,mouseenter:_4cf,mouseleave:_4d2,click:function(e){
var t=$(e.data.target);
...
$(this).tooltip("destroy");
}},rules:{length:{validator:function(_4f8,_4f9){
...
if(opts.multiline){
$("<textarea id=\""+_503+"\" class=\"textbox-text\" autocomplete=\"off\" spellcheck=\"false\"></textarea>").prependTo(tb);
}else{
$("<input id=\""+_503+"\" type=\""+opts.type+"\" class=\"textbox-text\" autocomplete=\"off\" spellcheck=\"false\">").prependTo(tb);
}
$("#"+_503).on('click', function() {$(this).focus();});
...
bc.append("<a href=\"javascript:;\" class=\"textbox-icon "+bb[i].iconCls+"\" icon-index=\""+i+"\" tabindex=\"-1\" title=\"" + (bb[i].title || "") + "\"></a>");
}
}
...
_512.css({width:opts.iconWidth+"px",height:_515+"px",'line-height':(_515+1)+"px"});
_512.find('.spinner-arrow-up').css('line-height', (_515+1)/2+"px");
_512.find('.spinner-arrow-down').css('line-height', (_515+1)/2+"px");
...
if($(this).hasClass("socui-fluid")||_526){
_50a(_51f);
}
return false;
});
...
cc.switchbutton("setValue",cc.switchbutton("options").offValue);
cc.each(function(){
if(_5d5($(this).switchbutton("options").defaultValue,val)){
$(this).switchbutton("check");
cc.switchbutton("setValue",val);
}
});
return true;
}
...
$.fn.form.defaults={fieldTypes:["tagbox","combobox","combotree","combogrid","combotreegrid","datetimebox","datebox","combo","datetimespinner","timespinner","numberspinner","spinner","slider","searchbox","numberbox","passwordbox","filebox","textbox","switchbutton","checkbox","radiobox","transfer"],novalidate:false,ajax:true,iframe:true,dirty:false,dirtyFields:[],url:null,queryParams:{},onSubmit:function(_5f0){
...
if($(this).hasClass("socui-fluid")||_60b){
_606(_60a);
}
return false;
});
...
if(menu.is(":visible")){
menu.hide();
$(this).find(".calendar-dtable").show();
}else{
_609(_60d);
}
}else{
...
if(_614){
menu.hide();
$(_60d).find(".calendar-dtable").show();
}
};
function _610(_616){
opts.year+=_616;
...
$(_618).find(".calendar-dtable").hide();
...
if ($(_626).find('.calendar-menu').is(':visible')) {
	$(_626).find('.calendar-dtable').hide();
}
...
var _67b=0;
function _67c(a,o){
return $socui.indexOfArray(a,o);
};
function _67d(a,o,id){
$socui.removeArrayItem(a,o,id);
};
function _67e(a,o,r){
$socui.addArrayItem(a,o,r);
};
...
var ss=["<style type=\"text/css\" socui=\"true\">"];
...
ss.push("</style>");
$(ss.join("\n")).appendTo(cc);
cc.children("style[socui]:not(:last)").remove();
},getRule:function(_688){
var _689=cc.children("style[socui]:last")[0];
...
$("<div class=\"datagrid-header-check\"></div>").html("<label><input type=\"checkbox\"/><i></i></label>").appendTo(td);
}else{
if(col.field){
td.attr("field",col.field);
...
var fixWidth = 7;
var checkboxWidth = 25;
var percentCol = 0;
for (var i=0; i<_730.length; i++) {
	var col=_6d9(_723,_730[i]);
	if (col.fix === true || col.field === '_expander') {
		fixWidth += col.width - 1;
	}
	// 当没有复选框时的列表偏移量
...
var delta = opts.scrollbarSize+(opts.rownumbers?opts.rownumberWidth:0)+fixWidth;
var viewWidth = dc.view.width();
//计算流体列宽度函数
function fluidColWidth(value) {
	delta = delta || 0;
	var v = $.trim(String(value||''));
	v = parseFloat(v.substr(0, v.length-1));
	v = (viewWidth-delta) * v / 100.0;
	return v;
}
$.map(_730,function(_731){
var col=_6d9(_723,_731);
if(String(col.width||"").indexOf("%")>=0){
var _732=fluidColWidth(col.width)-col.deltaWidth+(checkboxWidth/percentCol);
if(_732>0){
col.boxWidth=_732;
_72f=true;
}
...
cc.push(">");
if(col.checkbox){
cc.push("<label><input type=\"checkbox\" "+(_871.checked?"checked=\"checked\"":""));
...
var idx1=$socui.indexOfArray(rows,opts.idField,opts.lastSelectedIndex);
var idx2=$socui.indexOfArray(rows,opts.idField,_962);
...
ck.addClass("tree-checkbox"+flag);
if(flag==0){
$socui.removeArrayItem(_973,opts.idField,row[opts.idField]);
}else{
$socui.addArrayItem(_973,opts.idField,row);
}
};
function _96e(_974,row,_975){
var flag=_975?1:0;
_970(_974,row,flag);
$socui.forEach(row.children||[],true,function(r){
_970(_974,r,flag);
});
};
...
var _9a6=null;
$socui.forEach(_9a5.data,true,function(node){
...
$socui.removeArrayItem(_9cf.checkedRows,opts.idField,_9ce);
_95d(_9cd);
if(prow){
...
_9e6=_9e6||"checked";
var rows=[];
$socui.forEach(jq.data("treegrid").checkedRows,false,function(row){
...
var flag=0;
var crow=$socui.getArrayItem(_a02.checkedRows,opts.idField,row[opts.idField]);
if(crow){
flag=crow.checkState=="checked"?1:2;
row.checkState=crow.checkState;
row.checked=crow.checked;
$socui.addArrayItem(_a02.checkedRows,opts.idField,row);
}else{
var prow=$socui.getArrayItem(_a02.checkedRows,opts.idField,row._parentId);
...
if(row.checked){
$socui.addArrayItem(_a02.tmpIds,row[opts.idField]);
}
}
...
function _a91(_a92){
var _a93=_a82(_a92);
var opts = $.data(_a92,"combo").options;
if (opts.multiple && opts.multimodel) {
	var value = '';
	for (var val of _a93) {
		if (value) {
			value += opts.separator + val;
		} else {
			value = val;
		}
	}
...
var t=$(_aa0);
return $.extend({},$.fn.textbox.parseOptions(_aa0),$.parser.parseOptions(_aa0,["separator","panelAlign",{panelWidth:"number",hasDownArrow:"boolean",delay:"number",reversed:"boolean",multivalue:"boolean",multimodel:"boolean",selectOnNavigation:"boolean"},{panelMinWidth:"number",panelMaxWidth:"number",panelMinHeight:"number",panelMaxHeight:"number"}]),{panelHeight:(t.attr("panelHeight")=="auto"?"auto":parseInt(t.attr("panelHeight"))||undefined),multiple:(t.attr("multiple")?true:undefined)});
};
...
e.preventDefault();
e.stopPropagation();
}},panelWidth:null,panelHeight:300,panelMinWidth:null,panelMaxWidth:null,panelMinHeight:null,panelMaxHeight:null,panelAlign:"left",reversed:false,multiple:false,multivalue:true,multimodel:true,selectOnNavigation:true,separator:",",hasDownArrow:true,delay:200,keyHandler:{up:function(e){
},down:function(e){
},left:function(e){
},right:function(e){
},enter:function(e){
...
return $socui.indexOfArray(_aa6.data,_aa6.options.valueField,_aa5);
};
...
if(row){
if($socui.indexOfArray(_abe,row[opts.valueField])==-1){
...
$.map(_abe,function(v){
if($socui.indexOfArray(_abb,v)==-1){
...
var item=$socui.getArrayItem(a,opts.valueField,_ac1);
...
$socui.forEach(_ac5.data,false,function(row){
if(row["selected"]){
$socui.addArrayItem(vv,row[opts.valueField]+"");
}
});
if(opts.multiple){
_ab4(_ac3,vv,_ac4);
...
$socui.addArrayItem(opts.mappingRows,opts.valueField,_aea);
...
var _afd=$socui.indexOfArray($(_afb).combobox("getData"),opts.valueField,v);
if(_afd>=0){
vv.push(v);
}
});
...
var _b07=$socui.indexOfArray(_b06.groups,"value",_b05);
...
$socui.addArrayItem(_b16,node.id);
});
}
...
var item=$socui.getArrayItem(a,"id",_b27);
...
$socui.addArrayItem(opts.mappingRows,"id",_b35);
return _b35.id;
}else{
return _b35;
}
...
var item=$socui.getArrayItem(a,opts.idField,_b64);
return item?item[opts.textField]:undefined;
};
};
...
if($socui.indexOfArray(opts.mappingRows,opts.idField,v)>=0){
$socui.addArrayItem(vv,v);
}
});
...
$socui.addArrayItem(opts.mappingRows,opts.idField,_b76);
...
$socui.addArrayItem(_b81,row[opts.idField]);
});
}
_b86(_b7b,_b81);
...
var item=$socui.getArrayItem(a,opts.idField,_b8f);
...
if(q){
var v=undefined;
$socui.forEach(data,true,function(row){
...
if(v==undefined){
$socui.forEach(opts.mappingRows,false,function(row){
...
$socui.addArrayItem(opts.mappingRows,opts.idField,_b9d);
...
if($(this).hasClass("socui-fluid")||_c2e){
_c2f(_c2c);
}
return false;
});
