var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					cc.push('<label><input type="checkbox" name="' + field + '" value="' + (value!=undefined ? value : '') + '"><i></i></label>');
				} else if (col.expander) {
	...
	onAfterRender: function(target){
		...
		function resizeDetails(){
			...
			details.find('.socui-fluid').trigger('_resize');
			fr.show();
		}
			...
			function createGrid(target, conf, prow){
				...
				$(target).datagrid($.extend({}, conf.options, {
					...
					onExpandRow: function(index, row){
						...
						rd.find('.socui-fluid').trigger('_resize');
